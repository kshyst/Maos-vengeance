...
public class Tank extends Opp{
    public Texture texture = GameAssetsManager.gameAssetsManager.tankTexture;
    ...
    private void fireMissles(){
        if (Player.player.planeSprite.getY() - this.y < 500 * MaosVengeance.difficulty){
            ...
            if(timeSeconds > period){
                timeSeconds-=period;
                new OppMissle(x + texture.getWidth() / 2 , y + texture.getHeight() / 2 , 200 , 5 , new Texture(GameAssetsManager.gameAssetsManager.oppMissleTank));
            }
        }
    }
}
